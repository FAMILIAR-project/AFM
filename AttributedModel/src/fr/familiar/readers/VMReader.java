...
public class VMReader implements IReader {
	...
	public boolean fakeRealSupport = false;
	public Map<String, ArrayList<Object>> fakingRealValues = new HashMap<String, ArrayList<Object>>();
	...
	private Node<String> traverseConstraint(ComplexExpression cex) {
		...
		if (cex instanceof RightImplication) {
			...
			if (((PrimitiveExpression) cex).getFeatureID() != null) {
				...
				// System.out.println(((PrimitiveExpression)
				// cex).getFeatureID().getName());
			...
			if (((PrimitiveExpression) cex).getRefAtt() != null) {
				...
				node.addChild(new Node<String>(
						((PrimitiveExpression) cex).getRefAtt().getHead().getOwnedFeature().getName()));
				node.addChild(new Node<String>(((PrimitiveExpression) cex).getRefAtt().getName()));
				// System.out.println(((PrimitiveExpression)
				// cex).getRefAtt().getHead().getOwnedFeature().getName());
				// System.out.println('.'+((PrimitiveExpression)
				// cex).getRefAtt().getName());
			}
		} else {
	...
	private void visitAttributes(EList<AttrDef> att, AttributedFeatureModel fm) {
		...
		for (AttrDef atdef : att) {
			...
			if (at instanceof BooleanAttrDef) {
				...
				fr.familiar.attributedfm.Feature searchFeatureByName = fm
						.searchFeatureByName(((BooleanAttrDef) at).getName().getHead().getOwnedFeature().getName());
				...
				attribute.setName(name);
				fr.familiar.attributedfm.Feature searchFeatureByName = fm
						.searchFeatureByName(((StringAttrDef) at).getName().getHead().getOwnedFeature().getName());
				searchFeatureByName.addAttribute(attribute);
				...
				attribute.setDomain(domain);
				attribute.setName(name);
				fr.familiar.attributedfm.Feature searchFeatureByName = fm.searchFeatureByName(
				...
				fr.familiar.attributedfm.Feature searchFeatureByName = fm
						.searchFeatureByName(((RealAttrDefBounded) at).getName().getHead().getOwnedFeature().getName());
				searchFeatureByName.addAttribute(attribute);
			...
			if (enumAttrDef instanceof EnumStringDef) {
				...
				attribute.setName(name);
				fr.familiar.attributedfm.Feature searchFeatureByName = fm
						.searchFeatureByName(((EnumStringDef) enumAttrDef).getName().getHead().getOwnedFeature().getName());
				searchFeatureByName.addAttribute(attribute);
				...
				attribute.setName(name);
				fr.familiar.attributedfm.Feature searchFeatureByName = fm
						.searchFeatureByName(((EnumIntegerDef) enumAttrDef).getName().getHead().getOwnedFeature().getName());
				searchFeatureByName.addAttribute(attribute);
				...
				if (((EnumRealDef) enumAttrDef).getDefault() != null) {
					Float defaultvalue = Float.parseFloat(((EnumRealDef) enumAttrDef).getDefault().getValue());
					attribute.setDefaultValue(defaultvalue);
				}
				...
				attribute.setName(name);
				fr.familiar.attributedfm.Feature searchFeatureByName = fm
						.searchFeatureByName(((EnumRealDef) enumAttrDef).getName().getHead().getOwnedFeature().getName());
				searchFeatureByName.addAttribute(attribute);
			}
	...
	public Collection<fr.familiar.attributedfm.Constraint> parseConstratins(Collection<String> models, String ctc)
			throws Exception {
